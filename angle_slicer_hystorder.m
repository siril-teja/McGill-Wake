function [slice_points] = angle_slicer_hystorder(Data,slice_angle,tolerance) % slice_angle is radians, tolerance is degrees    
    ...
    [in_r_main,in_r_main_ind] = sort(in_r_main);
    in_z_main = Data.In_Mesh(in_idx_main,3);
    in_z_main = in_z_main(in_r_main_ind);
    %in_s_main = Data.Stiffness(in_idx_main);
    
    in_r_expl = -in_rho(in_idx_expl);
    [in_r_expl,in_r_expl_ind] = sort(in_r_expl);
    in_z_expl = -Data.In_Mesh(in_idx_expl,3);
    in_z_expl = in_z_expl(in_r_expl_ind);
    ...
    [out_r_main,out_r_main_ind] = sort(out_r_main);
    out_z_main = Data.Out_Mesh(out_idx_main,3);
    out_z_main = out_z_main(out_r_main_ind);
    %out_s_main = Data.Stiffness(out_idx_main);
    
    out_r_expl = -out_rho(out_idx_expl);
    [out_r_expl,out_r_expl_ind] = sort(out_r_expl);
    out_z_expl = -Data.Out_Mesh(out_idx_expl,3);
    out_z_expl = out_z_expl(out_r_expl_ind);
    %out_s_expl = Data.Stiffness(out_idx_expl);
    
    r_combined = [out_r_main; flip(in_r_main); flip(out_r_expl); in_r_expl];
    z_combined = [out_z_main; flip(in_z_main); flip(out_z_expl); in_z_expl];
    % if slice_angle>(pi/2)
